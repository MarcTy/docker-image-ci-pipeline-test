...
import { UseSettingsGlobalState, DEFAULT_TELEMETRY_ADDRESS_URL, 
    DEFAULT_TELEMETRY_FETCH_INTERVAL, DEFAULT_TELEMETRY_QOS } from '../../context/SettingsContext.jsx';
...
function TelemetryServerSetup () {
    ...
    const updateTelemetryServerParams = (url, interval, qos) => {
        if (telemetryServerAddressInputRef.current) {
            telemetryServerAddressInputRef.current.value = url;
        }
        if (intervalInputRef.current) {
            intervalInputRef.current.value = interval;
        }
        if (qosInputRef.current) {
            qosInputRef.current.value = qos;
        }
        dispatch({ type: 'UPDATE_TELEMETRY_SETTINGS', payload: { 
            telemetry_url: url, telemetry_interval: interval, telemetry_qos: qos} });
    }

    function ToDefaults () {
        updateTelemetryServerParams(DEFAULT_TELEMETRY_ADDRESS_URL, 
            DEFAULT_TELEMETRY_FETCH_INTERVAL, DEFAULT_TELEMETRY_QOS
        );
    }
    ...
    function FetchState() {
        ...
        const interval = state.telemetry_interval || DEFAULT_TELEMETRY_FETCH_INTERVAL;
        const qos = state.telemetry_qos || DEFAULT_TELEMETRY_QOS;

        updateTelemetryServerParams(url, interval, qos);
    }

    // Uses inputs to the form
    function SaveChanges () { 
        const url = telemetryServerAddressInputRef.current?.value || DEFAULT_TELEMETRY_ADDRESS_URL;
        const interval = intervalInputRef.current?.value || DEFAULT_TELEMETRY_FETCH_INTERVAL;
    ...
    return  (
        <div className="container">
            ...
            <div className="menu-content">
                ...
                <div className="form-group">
                    <label htmlFor="fetchIntervalFormInput">Fetch Interval (s)</label>
                    <input type="number" className="form-control" id="fetchIntervalFormInput" 
                    placeholder="5" ref={intervalInputRef}/>
                </div>
                <div className="form-group">
